...
| P0-FR-04 | Real commands, or a justified minimal harness, are defined for every phase | `v1/P0-BASELINE.md` section 3, `v1/P0-EVIDENCE-FORMAT.md` section 3 | The commands were run; exit codes in evidence/P0/logs/ | closed |
...
| P0-FR-06 | Evidence format carries exact SHA, session IDs, model contract, commands and exit codes | `v1/P0-EVIDENCE-FORMAT.md` sections 1 and 2 | The frozen record schema, with the P0 runs recorded in it | closed |
...
| P0-FR-08 | P0 does not change product semantics | the commit diff | Every changed path is `v1/**`; no file under `src/`, `migrations/`, `schema/`, `bin/`, `ci/`, `tools/`, `seed/`, `skills/` or `test/` is touched; full suite re-run green | closed |
