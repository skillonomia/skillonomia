...
| — | `GET /v1/console/webhooks` | a live console session (owner/admin) | — | `200 {"contract","items":[{"webhook_id","url","status","failure_count"}]}` — the caller's own endpoints, the same read `GET /v1/webhooks` serves, plus the console envelope. A `reviewer` session → `FORBIDDEN` |
