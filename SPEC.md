...
Version `0.1.6`. Licensed under Apache-2.0, together with the implementation in
this repository.
...
| — | `POST /v1/assignments/{assignment_id}/observations` | Bearer, workspace member | `{"observed_status","reason_code","reason","source","observed_at_ms"?,"revision_id"?,"session_ref"?,"provenance","idempotency_key"?}` | `201 {"assignment_id","observed"}`. The OBSERVED-state intake, and the only writer of it. `source` is one of `backend`, `adapter`, `runtime`; an owner console session cannot reach this route |
