...
Version `1.1.0`. Licensed under Apache-2.0, together with the implementation in
this repository.
...
attempt, server_at_ms}`, plus — when `kind` is `revocation` (`receipt_id` is
then `null`) — `revocation_reason`, `successor_version_id`,
`successor_semantic_version` and `registry_verification_path`. The two successor
members are `null` rather than omitted when the version names no replacement, so
a receiver can tell "there is no replacement" from "this server does not say".
`registry_verification_path` is the route on which the RECIPIENT may ask for a
verdict of its own — the stateless §4.4 form, which any authenticated principal
may call over bytes it already holds, and not the owner/admin transition form a
notified adopter would be `FORBIDDEN` from calling. All four are read when the
notice is PUSHED, so a successor attached after the revocation reaches an
adopter whose delivery had not yet succeeded. It carries an HMAC-SHA256
...
that IS this machine — a loopback literal, or a name in the reserved `localhost`
space — and not to a host whose name begins with those characters.

The reserved space is the WHOLE of it. RFC 6761 section 6.3 reserves `localhost.`
and `*.localhost.` in one breath, and gives them one property: they resolve to a
loopback address or to nothing. A registry that reads the bare label out of that
section and stops admits `foo.localhost`, which the same host's resolver answers
with `::1`. The test a conforming registry applies is therefore over the
canonical host: it IS `localhost`, or it ENDS IN `.localhost`. Being a suffix
test and not a substring one, it does not reach `localhost.attacker.com`, which
is a name the attacker controls and remains an ordinary host.
...
registration judges it under rule 4 immediately. A RESERVED `localhost` name is
not an exception to the no-resolution rule but an application of the same
reasoning: it names this machine by definition, and registration therefore judges
it under rule 4's loopback clause immediately as well, under either scheme. This
reaches reserved names only. An unreserved name such as `evil.example.com` MAY
resolve to a loopback address tomorrow; registration cannot know that and does
not claim to, and rule 4 at the socket is what refuses it.
...
**The envelope announces two vocabularies for status and two for kind, and they
are not interchangeable.** `statuses` and `kinds` are the values an ITEM may
carry; `status_filters` and `kind_filters` are the values the QUERY accepts.
`decided` is a filter and never a status — no item is ever `decided` — and
`conditional` is a status and never a filter. A conforming console builds its
filter controls from `status_filters` and `kind_filters`, because a control built
from the item vocabulary offers an operator a filter this route answers
`INVALID_SCHEMA` to, and inventing the filter vocabulary in a client is the
second source of truth §6.4 exists to prevent.
...
- An endpoint naming a loopback host — a loopback literal, its IPv4-mapped IPv6
  form, or a name RFC 6761 section 6.3 reserves, which is `localhost` itself and
  every name under `.localhost` — is admitted ONLY where the process's transport
...
D.1o adds to `idempotency_keys` — so any further divergence
  fails.
...
EVERY CHANNEL INTO THIS COLUMN IS CLOSED, and none of them is closed in SQL. A
report may present only the names THE REGISTRY'S OWN CHECKS READ — the union of
the `evidence` inputs of the check kinds, four names in this version — and under
...
string an author wrote is a VALUE. AND NO STRING AN AUTHOR WROTE IS A NAME
EITHER, which is the rule that closes the last of the three and took two attempts.
A name a signed contract declares is a string the author wrote, and while a
declaration WIDENED the admissible names it was stored in this column as a KEY:
the first answer was to constrain its FORM to an identifier, and a form is not a
subject — a hex string of 33 characters is an identifier, and one went into this
column through the shipped route. A conforming registry MUST therefore admit ONLY
the names its own checks read, and MUST NOT let a version's signed
`outcome_contract.evidence` add to them: that field states what the AUTHOR wants a
run to present and IS NOT A SOURCE OF ADMISSIBLE NAMES for this journal. It stays
in the signed manifest with the standing the substring form of `stdout_match` has
— [I-7] bounds the journal and not a document under a signature — and the form
required of it (a lowercase letter, then lowercase letters, digits and
underscores, at most 40 characters, in the SCHEMA and again wherever a contract
is read) is a rule about a MANIFEST being machine-readable, not the rule that
protects this column. A name is deliberately NOT reduced to a digest: a reader of
the journal must see WHICH quantity was presented, which is affordable exactly
because the names are the registry's own. What is NOT promised, stated rather
than glossed: an author still chooses the strings inside its own manifest, and any
bounded alphabet can be made to carry an encoding as a flat list of integers can
— what is delivered is that the registry puts no text in this column and the
names it accepts there are its own. A consequence, stated because
...
#### The journals, column by column — the surface, enumerated

Four channels by which a caller's text reached a journal were found ONE AT A TIME
by successive passes of review, and each closing note said the class was closed.
The cause was never the quality of a fix: THE SURFACE WAS NEVER ENUMERATED. A
conforming registry MUST therefore hold a CLASSIFICATION OF EVERY COLUMN OF EVERY
JOURNAL, and MUST take the set of columns from its own schema rather than from a
list kept by hand.

A JOURNAL is a table the schema marks INSERT-only. Its columns come from the
schema itself. Each is one of:

- **registry-generated** — the caller does not influence the value;
- **a bounded form** — an enum, an identifier, a number or a boolean;
- **a digest** — stored as `sha256:<64 lowercase hex>`;
- **a declared limit** — a caller's text is admitted ON PURPOSE, and the limit is
  stated in the files the package ships.

A column present in the schema and absent from the classification MUST FAIL THE
BUILD. Free text with no decision behind it is not a fifth class: it is the
defect this rule exists to make impossible to file quietly. In this
implementation the classification is `JOURNAL_INTAKE` (`src/journal.ts`), the
column set is read from `sqlite_master` and `PRAGMA table_info`, and a probe adds
a column to a journal of a scratch database to show that the guard reports it.

`observed_records.call_id` and `receipt_events.idempotency_key` ARE DIGESTS. Both
exist so that two rows can be recognised as the same fact — a call and its output
[M-5], a retry and the call it repeats — and neither is ever read. Equality is
exactly what a digest preserves, so the correlation is untouched and the caller's
string is not stored; a reader holding the runtime's own log hashes the id they
have and finds the row. A `call_id` that was absent stays NULL and MUST NOT be
hashed: a digest of an absence would give every such record one shared value and
manufacture pairs out of nothing, which is `unknown` turned into `yes` [I-1],
[A-0].

`runtime_observations.window_detail` IS COMPOSED BY THE REGISTRY. A reporter
states its boundary in bounded parts — the `window` kind, and for a `period` the
milliseconds `window_from_ms` and `window_to_ms` — and the registry writes the
phrase. A report that sends `window_detail` is REFUSED and not ignored, because a
reporter whose boundary was silently dropped would believe it had been recorded.
This is stricter about [I-3] than the prose it replaces: a boundary is now
machine-readable, and a `period` that declares no bounds is refused rather than
described in words nothing can check. `runtime_observations.model` is a NAME OF A
MODEL — at most 64 characters, no whitespace, refused rather than truncated —
and that is a narrowing rather than a closure, because a form is not a subject.

THE DECLARED LIMITS OF V-1 ARE FOUR COLUMNS OF `receipt_events`, and they are
named here rather than left to be discovered: `evidence_json`,
`failure_report_json`, `rollback_report_json` and `environment_json`. §5.3
requires an adopter to present the results of the gates a version declared, to
say WHY an adoption failed, to account for what a rollback undid, and to declare
the environment it ran in. Every one of those is a document a HUMAN reads in
order to decide whether to trust an adoption, and a digest of a report is not a
report. Their SHAPE is schema-checked; the prose inside their own fields is the
adopter's, and an adopter is a fleet agent like any other. That is a limit of
V-1, stated, and not a channel anybody overlooked.

What is delivered, with its own exception inside the sentence rather than in a
footnote to it: EXCEPT IN THE COLUMNS A REGISTRY NAMES AS DECLARED LIMITS, IT
PUTS NO CALLER'S TEXT INTO A JOURNAL, AND THE FORMS IT ACCEPTS THERE ARE ITS OWN.
The larger sentence — that a secret cannot be in a journal — is false under every
alphabet and MUST NOT be written: a bounded alphabet can be made to carry an
encoding, and a bound on a number bounds a quantity of bits and not their
meaning.
...
### D.1k NORMATIVE DELTA — eleventh migration (verbatim)

D.1j's rule for the journals was applied to the WRITER and to the READER of
`receipt_events.idempotency_key` and to no existing row. A registry that upgrades
a database written by an earlier build therefore held the adopter's key in the
...
redaction preview a draft carries states the CATEGORY, the LOCATION — the field
it came from as a structured value as well as the line and column — and the
REASON of everything removed, and MUST NOT carry the value or a digest of it.

THE ONE FIELD OF A CAPTURE THAT CANNOT BE REDACTED IS DIGESTED INSTEAD. An
`idempotency_key` is the index a repeat is found by, so a cleaned key would no
longer equal the key the retry sends and the replay would be lost. On
`capture.submit` and `draft.revise` — and on no other surface —
`idempotency_keys.key` therefore holds `sha256:<hex>` of the caller's key rather
than the key: equality survives a hash exactly, the string does not survive at
all, and a caller who put a credential in the key of a capture has not made it
durable. The rule is by SURFACE and never by the form of a value, for the reason
Appendix D.1l gives.

A BOUND ON A STORED COLUMN IS CHECKED AFTER THE TEXT IS CLEANED. Redaction
replaces material with a marker and can make a value LONGER than it arrived, so
a `source_ref` inside the published input bound can be outside the column's, and
an empty source is outside the lower bound of `captures.redacted_source`. A
CONFORMING REGISTRY MUST answer each of those as a typed result — `LIMIT_EXCEEDED`
for a reference it cannot store, a structured refusal with `EMPTY_SOURCE` or
`SOURCE_TOO_LARGE` for a source it cannot store — and MUST NOT let a bounded,
accepted capture arrive at a constraint and return an internal error.
...
command: `assignment_observations.source` is one of `backend`, `adapter` or
`runtime` and has no `owner` member, and the CHECK is where that is enforced.
Every observation, `unknown` included, MUST carry `reason_code`, `reason`,
`source` and `observed_at_ms`; all four are NOT NULL. The absence of any
observation is itself `unknown`, computed by the reader and stored nowhere.

AN OWNER COMMAND APPLIES TO THE NEXT SESSION. `effective_from` is on every
lifecycle event and its only permitted value is `next_session`; a conforming
registry MUST NOT rewrite the loadout of a session that has already started.
`event_seq` is dense from 1, UNIQUE per assignment, and is the optimistic
concurrency token a caller states in `if_version`: a stale one is
`PRECONDITION_FAILED` with the current state.

AN APPROVAL IS A FACT ABOUT A REVISION. `revision_approvals.draft_revision_id`
is UNIQUE, so one revision is approvable exactly once, and a lineage may carry
more than one approved revision — which is what a rollback selects among.
`draft_decisions` of D.1n is NOT altered and keeps its `UNIQUE(draft_id)`: it
    ...
    "outcome_contract": {"type": "object", "additionalProperties": false,
      "required": ["check","evidence","unknown"], "properties": {
        ...
        "evidence": {"type": "array", "minItems": 1, "maxItems": 20,
          "description": "DECLARATIVE, AND THE AUTHOR'S: the named values a run of this skill ought to present. A registry MUST NOT read it as a source of admissible names for its observation journal — the names a report may present are the ones the registry's own checks read, and a name declared here is never a key of a record. The identifier form is required so that a signed manifest stays machine-readable, and it is not a guarantee about the journal: an identifier is a string the author chose, and any bounded alphabet can be made to carry an encoding.",
          "items": {"type": "string", "pattern": "^[a-z][a-z0-9_]{0,39}$", "maxLength": 40}},
...
| 10 | `skill.supersede` | `POST /v1/versions/{id}/supersede` | author/skill owner/reviewer/admin/owner | `{"successor_version_id","idempotency_key"?}` | `200 {"skill_version_id","state","superseded_by","successor":{"skill_version_id","state"},"tlog_seq","noop"}` — both versions' lifecycle links written atomically with the `version_superseded` entry. `state` is `superseded` when the subject was `published`; a subject already in a tail KEEPS its disposition — `deprecated` stays `deprecated` and `revoked` stays `revoked` — because recording a replacement is a column write and not a state change (§5.1b). Already linked to this successor → the same body minus `tlog_seq`, plus `"noop":true` and the RECORDED `superseded_by`; already linked to a different one → `CONFLICT{current_state}`. A successor that is not itself `verified`/`published` → `PRECONDITION_FAILED{current_state}`; a successor whose signed `lifecycle.supersedes` names a different version, or which already supersedes another → `CONFLICT{current_state}`; a successor of a different skill, or equal to the subject → `INVALID_SCHEMA`; a subject in a state that may hold no link at all → `PRECONDITION_FAILED{current_state}` |
| 11 | `skill.revoke` | `POST /v1/versions/{id}/revoke` | author/skill owner/admin/owner | `{"reason","successor_version_id"?,"idempotency_key"?}` — `reason` a non-empty string of ≤2000 characters, or `INVALID_SCHEMA`; `successor_version_id` is OPTIONAL and additive, and an absent one is the same request as an explicit `null` — never a request to remove a link already recorded | `200 {"skill_version_id","state":"revoked","reason","superseded_by","notifications_queued","notified_adopters","tlog_seq","lineage_tlog_seq","noop"}`. `superseded_by` is ALWAYS present (`string\|null`), so "no replacement" is distinguishable from "this server does not say". `notified_adopters` keeps its v1.0.0 meaning — the count of §5.2 revocation notices QUEUED in the same transaction, never proof of delivery — and `notifications_queued` is the same count under the honest name; both are present on a fresh revoke and absent on a call that queued nothing. `tlog_seq` always means the `version_revoked` entry and `lineage_tlog_seq` appears only when THIS call appended a `version_superseded` entry after it. Any released state (`published`, `deprecated`, `superseded`) may be revoked; a subject in an earlier state → `PRECONDITION_FAILED{current_state}`. Already revoked with the SAME reason and no new link → the same body plus `"noop":true`, the ORIGINAL reason, and no second tlog row or notice; already revoked with a DIFFERENT reason, or naming a successor other than the one recorded → `CONFLICT{current_state}` and nothing is written. A successor of a different skill, or equal to the subject → `INVALID_SCHEMA`; one not itself `verified`/`published` → `PRECONDITION_FAILED{current_state}` |
...
| `observation.report` | `POST /v1/observations` | member (holder of a `report_outcome` grant, §6.2) | `{"agent_id","runtime":"claude_code\|codex","window":"live_session\|period\|all_time","window_from_ms"?,"window_to_ms"?,"model"?,"session_active"?,"last_activity_ms"?,"proposal_inventory_complete"?,"records":[{"role":"proposal\|call\|output","call_id"?,"at_ms"?,"marker"?,"text"?,"result":"success\|failure\|unknown"?}…]?,"idempotency_key"?}` — `window` is REQUIRED and a `period` report is REQUIRED to state `window_from_ms` and `window_to_ms`: a report that does not say what it looked at is a number with no method ([I-3]), and a default would describe the wrong search. The BOUNDARY PHRASE is composed by the registry from those bounded parts; `window_detail` on INPUT is `INVALID_SCHEMA`, refused rather than ignored, because a reporter whose boundary was silently dropped would believe it had been recorded. `model` is a name of a model (at most 64 characters, no whitespace) and is refused rather than truncated; omitting it reports `unknown`, which is not `no model` ([I-1]). `records[].call_id` is stored as `sha256:` of itself — [M-5] asks only whether two records share an id, and a record whose runtime gave none stays NULL and can never form a pair | `201 {"observation_id","agent_id","runtime","records_examined","markers_recorded","window","window_detail","records_text_stored":false,"note"}`. **THIS SURFACE WRITES.** The V-1 requirements list it among the READING surfaces; a self-report is an agent TELLING the registry something, telling is storing, and this call appends to two INSERT-only tables and moves the observation column of every deployment of that agent. Its MCP annotations MUST therefore be those of a write (`readOnlyHint:false`), because [I-8] requires a tool's hints to be true and a client acts on a false one by not asking. The records' TEXT is reduced to §5 arrival markers at this boundary and is NOT stored, logged or returned ([I-7]); `records_text_stored` states that rather than leaving it to be inferred. A report can establish that a version RAN; it can NEVER establish that one did not — a marker absent from a report is `unknown`, never `no`. An agent of another workspace → `NOT_FOUND`; no `report_outcome` grant → `FORBIDDEN`; more than 5000 records → `LIMIT_EXCEEDED` |
...
| — | `GET /v1/console/fleet` | a live console session (owner/admin) | — | `200 {"contract","agents":[{"agent_id","name","type","status"}]}` — the ACTIVE agents of the caller's own workspace, which is the closed fleet an assignment may name |
| — | `GET /v1/console/capabilities` | a live console session (owner/admin) | — | `200 {"contract","items":[{"draft_id","title","latest_revision_id","latest_revision","approved_revisions","head_approval","assignment_count","active_assignment_count"}]}` — every lineage carrying at least one approved revision |
| — | `GET /v1/console/capabilities/{draft_id}` | a live console session (owner/admin) | — | `200 {"contract","draft_id","title","approved_revisions","head_approval","fleet","eligibility","assignments","effective_from","desired_state_source","observed_state_source","lifecycle_states"}`. `eligibility` carries the server's verdict per fleet agent; a console MUST render it rather than compute one |
| — | `POST /v1/console/assignments` | a live console session (owner/admin) | `{"agent_id","revision_id","reason"?,"idempotency_key"?}` | `201 {"contract","assignment","effective_from":"next_session","event_id"}`. Only an APPROVED revision and only an active agent of the caller's own workspace: anything else is `PRECONDITION_FAILED` carrying the eligibility `reason_code` as `current_state`. A repeat with the same key and the same payload replays; the same key with a different payload is `CONFLICT` |
| — | `GET /v1/console/assignments/{assignment_id}` | a live console session (owner/admin) | — | `200 {"contract","assignment":{"assignment_id","agent_id","draft_id","workspace_id","desired","observed","actions","approved_revisions","entity_version","created_at_ms"},"effective_from","desired_state_source","observed_state_source"}`. `desired` and `observed` are separate objects and neither is computed from the other |
| — | `POST /v1/console/assignments/{assignment_id}/activate` | a live console session (owner/admin) | `{"if_version"?,"reason"?,"idempotency_key"?}` | `200` — the assignment view. A transition the lifecycle does not allow is `PRECONDITION_FAILED` with `current_state`; a request for the state the assignment is already in is a convergent noop. A revoked assignment is NOT reactivated |
| — | `POST /v1/console/assignments/{assignment_id}/pause` | a live console session (owner/admin) | `{"if_version"?,"reason"?,"idempotency_key"?}` | `200` — the same shape and the same rules as `activate` |
| — | `POST /v1/console/assignments/{assignment_id}/revoke` | a live console session (owner/admin) | `{"if_version"?,"reason"?,"idempotency_key"?}` | `200` — the same shape. `revoked` is terminal: taking the capability back again is a new assignment |
...
| — | `GET /v1/console/versions/{version_id}/revocation` | a live console session (owner/admin) | — | `200 {"contract","skill_version_id","skill_id","slug","semantic_version","manifest_hash","state","revocation_reason","superseded_by","active_adopters","successors","notices","eligibility","server_at_ms"}` — what revoking this version would do, READ BEFORE ANYTHING IS WRITTEN, so the console can state the consequences §6.4 requires it to state before commit. `active_adopters` is `[{"adopter_agent_id","receipts"}]` and is the set the revoke call would queue a notice for. `successors` is `[{"skill_version_id","semantic_version","state"}]` — the `verified` or `published` versions of the same skill, never this one. `notices` is `{"queued","delivered","dead_lettered","total"}`, three separate counts under three separate names: `queued` counts notices the delivery machine still holds and is not evidence that anything arrived. `eligibility` is `{"allowed","reason_code"}` computed from the same state set and the same actor rule the revoke call applies. A version of another workspace is `NOT_FOUND` |
| — | `POST /v1/console/versions/{version_id}/revoke` | a live console session (owner/admin) | `{"reason","successor_version_id"?,"idempotency_key"?}` | `200 {"contract",…the body of `skill.revoke`…}`. A wrapper over surface 11 and nothing else: the route checks the session, the `Origin`, the CSRF token and the §6.4 route ACL, then calls the same service method the Bearer route calls with the session's own `AuthContext`. `reason` is required and non-empty; `successor_version_id` is optional and admits `null` for no replacement. The state whitelist, the actor ACL, the immutability of the reason, the lineage link, the transparency-log order and the idempotent replay are the service's and are not recomputed here. A `reviewer` session → `FORBIDDEN` |
| — | `GET /v1/console/webhooks` | a live console session (owner/admin) | — | `200 {"contract","items":[{"webhook_id","url","status","failure_count","eligibility":{"allowed","reason_code"}}]}` — the caller's own endpoints, the same read `GET /v1/webhooks` serves, plus the console envelope and, per row, this deployment's verdict on the one action the console offers for it. `eligibility` is ADDITIVE and belongs to the console read alone: `GET /v1/webhooks` is a machine surface whose caller decides for itself what to call next, and INV-09 keeps its body unchanged. The verdict is `{allowed:true, reason_code:"TESTABLE"}` where the stored URL is admissible under the policy this process's transport declares, and `{allowed:false, reason_code:"ENDPOINT_NOT_DELIVERABLE"}` where it is not — judged by `vetEndpointUrl` under `registrationUrlPolicy`, the same two functions registration uses, so the offered control, the registration surface and the delivery path read one rule (§6.5.1, INV-08). A row outlives the policy that admitted it: an `http://127.0.0.1` endpoint registered while `SKILLONOMIA_WEBHOOK_ALLOW_LOOPBACK` is set names, after a restart without it, a destination this deployment will not open a socket to. `status` is NOT consulted: a `dead` endpoint stays testable on purpose, which is how an operator learns a repaired receiver answers again. The verdict decides whether the console OFFERS the control; `POST /v1/console/webhooks/{webhook_id}/test` still decides whether to act. A `reviewer` session → `FORBIDDEN` |
| — | `POST /v1/console/webhooks` | a live console session (owner/admin) | `{"url","idempotency_key"?}` | `201 {"contract","webhook_id","url"}` — registration through the console. The URL policy, the SSRF rules, the loopback flag and the secret's generation are the same service call `POST /v1/webhooks` makes, which is what §6.5.1's registration parity means. **The plaintext secret is NOT in this response**: a browser has no use for it and INV-04 forbids a credential reaching the DOM, so the console wrapper drops it, and the idempotency record is written over the stripped value so no replay can hold one either. A `reviewer` session → `FORBIDDEN` |
