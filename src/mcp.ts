...
export const MCP_TOOLS = [
  ...
  },
  {
    ...
    description:
      "§6 part A: file what was OBSERVED at one agent's runtime — the model, whether a session is live, when it was last active, and the runtime records themselves. THIS TOOL WRITES. The V-1 requirements list it among the READING surfaces and that classification is contradicted here deliberately: a self-report is an agent telling this registry something, telling is storing, and this call appends to two INSERT-only tables and moves the observation column of every deployment of that agent. [I-8] requires a tool's hints to be TRUE, and `readOnlyHint: true` on a call that writes is a false hint a client acts on by not asking. Requires a `report_outcome` grant scoped to `local_agent` (§6.2). `window` is REQUIRED, and a `period` report is REQUIRED to state `window_from_ms` and `window_to_ms`: a report that does not say what it looked at is a number with no method [I-3], and it is refused rather than given a default. THE BOUNDARY PHRASE IS COMPOSED BY THIS REGISTRY from those bounded parts and is not a sentence a reporter writes — `window_detail` is REFUSED on input rather than ignored, because a reporter whose boundary was silently dropped would believe it had been recorded. `model` is a name of a model — at most 64 characters, no whitespace — and is refused rather than truncated; omit it to report `unknown`, which is not `no model` [I-1]. `records[].call_id` is stored as `sha256:` of itself: [M-5] asks only whether two records carry the SAME id, equality is what a digest keeps, and a record whose runtime gave no id stays NULL and can never form a pair. The records' TEXT is reduced to §5 arrival markers at this boundary and is NOT stored, logged or returned [I-7]. A report can establish that a version RAN; it can never establish that one did not. A record MAY state a `result`, and a stated `success` or `failure` REQUIRES `evidence` — an object of named values. The ADMISSIBLE NAMES are the ones this registry's four checks read (`exit_code`, `stdout_sha256`, `artifacts`, `command`) AND NO OTHERS; any other name is `INVALID_SCHEMA` and nothing is stored [I-7]. THE VALUES ARE BOUNDED TOO: a named value is a boolean, a safe integer or a digest of the form `sha256:<64 lowercase hex>` — or a FLAT list of at most 32 of those, with NO NESTING at any depth, because a bound applied to each level of a tree is a bound on no value at all. Free text is `INVALID_SCHEMA` under EVERY name, the contract's own included, and so is a tree of it: a journal that takes arbitrary text holds transcripts however its fields are named and however the text is spread across them [I-7]. Where the signed `check` names a `command` or an `artifact_path`, a run presents THE DIGEST of it and the registry compares that against the digest it computes from the signed parameter itself, so the check decides exactly what it decided before while the author's own words — an author is a fleet agent too — never enter the journal as a VALUE. AND THE NAMES ARE THE REGISTRY'S OWN, which is the rule that replaced a rule that did not hold. `free text is refused under every name` is true of what a name CARRIES and says nothing about what a name IS, and a NAME the signed contract declared was itself a string the author wrote, stored as written, as a key. The answer was to require the FORM of an identifier — a lowercase letter, then lowercase letters, digits and underscores, at most 40 characters — and a form is not a subject: `a0123456789abcdef0123456789abcdef` is an ordinary hex string of 33 characters and is inside that form, so it went into the journal through this very surface. Every alphabet fit for readable names is fit for part of the secrets, so the widening itself is gone: `outcome_contract.evidence` is NOT A SOURCE OF ADMISSIBLE NAMES here. It remains in the signed manifest as the author's declaration of what a run ought to present — the standing the substring form of `stdout_match` has, because [I-7] bounds this journal and not a document under a signature — and the identifier form is still required of it so that a manifest stays machine-readable. A name in the journal is deliberately NOT reduced to a digest: you have to be able to see WHICH quantity a run presented, and that is affordable because the four names are this registry's own. What is not promised is said rather than promised away: an author chooses the strings in its own manifest, and any bounded alphabet can be made to carry an encoding, so what is delivered is that this registry puts no text into this journal and the forms it accepts there are its own. Every column of every INSERT-only journal is classified in `src/journal.ts`, the set of columns is read from the schema, and a column nobody has classified fails the build. The output channel is `stdout_sha256` and not `stdout` because a digest is all it can hold, and a field named for text it cannot carry would be asserting more than this registry delivers. The verdict is not taken from the word: the registry executes the contract's own `check` against the evidence, so a principal holding this grant states what it OBSERVED and never what the outcome WAS [M-6], [D-2]. And it is not taken from the evidence alone either: values reported about a process on the ADDRESSEE'S machine are a SELF-REPORT, so the §4 `outcome` column publishes `unknown` with a reason, beside the conclusion that report amounts to, who claimed it and of which principal type — never as this registry's `yes` [M-7], [D-18]. `yes` and `no` are published only where the registry read the thing itself, which today means an artifact its OWN ACTIVATION JOURNAL records it placed under the activation root it manages — a file it did not put there is not a file it can answer for. Without an executed check the column reads `unknown` with a reason, never `no`.",
    ...
    inputSchema: {
      ...
      properties: {
        ...
        window_from_ms: { type: "number" },
        window_to_ms: { type: "number" },
      ...
      required: ["agent_id", "runtime", "window"],
    },
  },
  {
